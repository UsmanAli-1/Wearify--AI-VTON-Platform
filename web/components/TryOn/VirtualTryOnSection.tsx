...
import { faDownload, faXmark, faImages } from "@fortawesome/free-solid-svg-icons";
...
export default function UploadTryOnSection() {
  ...
  useEffect(() => {
    ...
    if (b64 && gId) {
      fetch(b64).then((r) => r.blob()).then((blob) => {
        setSelectedFile(new File([blob], name || "photo.jpg", { type: blob.type }));
        setUploadedImage(b64);
      });
      setSelectedGarment({ _id: gId, name: gName!, imagePath: gImg! });
      ["prefill_person_base64","prefill_person_name","prefill_garment_id","prefill_garment_image","prefill_garment_name"]
  ...
  const handleImageCheck = async () => {
    ...
    if (!res.ok) {
      ...
      if (data.reason) toast.error(reasonMap[data.reason] ?? "Invalid image");
        ...
        if (!upgradeShownThisSession) { setUpgradeShownThisSession(true); setShowUpgrade(true); }
    ...
    if (data.pointsExhausted && !upgradeShownThisSession) {
      setUpgradeShownThisSession(true); setShowUpgrade(true);
    }
  };
  ...
  const buttonContent =
    generatingPhase === "scanning"   ? <><Spinner /> Scanning Image...</>  :
    generatingPhase === "generating" ? <><Spinner /> Generating Image...</> :
    <>✨ Try On</>;

  const TryOnBtn = () => (
    <Button disabled={!isLoggedIn || generating} onClick={handleImageCheck}
  ...
  /* Inner content of the generated-preview card — identical in both layouts */
  const GeneratedInner = () => (
    <div className="absolute inset-3 border-2 border-dashed border-white/20 rounded-2xl overflow-hidden flex items-center justify-center">
      {generatedImage ? (
        <>
          <img src={generatedImage} alt="generated" className="absolute inset-0 w-full h-full object-cover rounded-2xl" />
          <div className="absolute top-2 right-2 z-10 flex gap-2">
            <a href={generatedImage} download="wearify-tryon.jpg"
    ...
    <>
      {uploadedImage ? (
        ...
        <div className="flex flex-col items-center gap-1 text-center px-3">
          ...
          {!isLoggedIn && <a href="/signin" className="text-xs text-[#A06CE3] mt-1">Sign in to upload</a>}
        </div>
      )}
    </>
  );

  /* Reusable card content for Selected Garment */
  const GarmentCardInner = ({ hint }: { hint: string }) => (
    <>
      {selectedGarment ? (
        <div className="relative w-full h-full">
          <img src={selectedGarment.imagePath} alt={selectedGarment.name} className="absolute inset-0 w-full h-full object-cover rounded-xl" />
    ...
    <section className="w-full flex flex-col sm:h-[calc(100vh-35px)] md:h-[calc(100vh-65px)] sm:pt-20 md:pt-0 px-5 sm:px-5 md:px-12 xl:px-20 sm:overflow-hidden">
      ...
      <div className="flex flex-col gap-3 pt-22 pb-5 sm:hidden overflow-y-auto">
        ...
        <Card className="rounded-2xl bg-white/5 backdrop-blur-md border border-white/10 shadow-xl overflow-hidden py-2 px-2">
          <div className="flex gap-2 overflow-x-auto no-scrollbar items-center">
            {garments.map((g) => (
              <img key={g._id} src={g.imagePath} alt={g.name}
      ...
      <div className="hidden sm:flex flex-col gap-3 h-full  md:pb-10">
        ...
        <div className="flex-1 min-h-0 grid grid-cols-3 lg:grid-cols-4 gap-3 md:gap-4 overflow-hidden">
          ...
          {/* ── COL 1: Garments panel ──
              sm–md: slightly narrower, lg+: normal width              */}
          ...
          {/* ── COL 2 (sm–md): Upload + Selected stacked
                 at lg+ this div becomes invisible and the two cards below take over ── */}
      ...
      <UpgradeModal
        isOpen={showUpgrade}
        onClose={() => { setShowUpgrade(false); setUpgradeShownThisSession(false); }}
      />
      <TryOnGalleryModal isOpen={showGallery} onClose={() => setShowGallery(false)} />
    </section>
  );
}
